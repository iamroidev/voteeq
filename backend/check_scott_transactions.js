...
async function main() {
  ...
  if (nominee) {
    ...
    const txs = await db.all("SELECT * FROM votes WHERE candidate_code = '3405' OR nominee_id = ?", [nominee.id]);
    console.log(`Found ${txs.length} transactions:`);
    txs.forEach(t => {
      console.log(`ID: ${t.id} | Reference: ${t.reference} | Vote Count: ${t.vote_count} | Status: ${t.status} | Created: ${t.created_at} | Phone: ${t.phone} | Email: ${t.email}`);
    });
  } else {
